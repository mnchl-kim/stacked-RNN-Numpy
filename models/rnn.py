...
def rnn_cell_forward(xt, a_prev, parameters):
    """
    Implements a single forward step of the Sequence Models-cell.
...
def rnn_forward(X, Y, a0, parameters):
    ...
    # Store values needed for backward propagation in caches
    # Caches contain every time-step cache values
    # cache = (a_next, a_prev, xt, parameters) at time t
    caches = []
...
def rnn_cell_backward(da_next, cache):
    """
    Implements the backward pass for the Sequence Models-cell (single time-step).
    ...
    # compute the gradient of loss with respect to tanh
    dtanh = da_next * (1 - a_next ** 2)

    # compute the gradient of the loss with respect to xt
    dxt = np.dot(Wax.T, dtanh)

    # compute the gradient of the loss with respect to a_prev
...
def rnn_backward(da, caches):
    """
    Implement the backward pass for a Sequence Models over an entire sequence of input data.
